...
이 글 자체가 OpenClaw로 만들어졌다.

셋업 끝나고 Telegram에서 이런 대화가 오갔다:

```
나: 시험삼아서 하나 해보자. 마이크로블로그 하나 만들어서 vercel에 배포해보자

맥북: 좋아! 🚀
      프레임워크: Next.js로 갈까?
      데이터: 일단 로컬 JSON 파일?

나: 클로드나 챗지피티로 마크다운 작성하고 옵시디안으로 정리한다음 
    내가 너한테 게시하라고 하면 배포하는식으로 할거야

맥북: 아 이해했어! 옵시디안 vault 위치가 어디야?

나: 옵시디안 vault를 새로 구성해보자

맥북: 좋아! 새 vault 만들자.
```

그렇게 10분 만에:
- `~/Documents/microblog-vault/` 생성
- Next.js 블로그 프로젝트 생성
- GitHub 연결
- Vercel 배포
...
```
나: Application error: a client-side exception has occurred

맥북: 아 맞다! 문제 발견 💡
      지금 블로그가 로컬 경로에서 읽는데, Vercel 서버에는 그 폴더가 없어.
      수정할게:
```
...
```
나: 오브젝트는 올바른 리액트 차일드가 아니래

맥북: (Date 객체를 문자열로 변환하는 코드 수정)
      다시 확인해봐
```

3번의 수정 끝에 블로그가 떴다.

그리고 Claude.ai에서 작성한 이 셋업 가이드를 Telegram으로 붙여넣었더니, 맥북이 알아서 마크다운 파일로 저장하고 배포했다.

**지금 보고 있는 이 글이 그 결과물이다.**

```
나: 이 글 마지막에 지금 이 대화에서 나랑 블로그 만든 과정을 
    추가해서 메타적인 글로 좀 재밌게 해보자
